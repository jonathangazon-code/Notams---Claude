...
namespace ICAO_CSV
{
	public partial class MainForm
	{
		...
		void tab_RWYs()
		{
			...
			bReimport.Click += (s, e) => { if (!EnsureWriterOrWarn()) return; if (_rwyCmb.Text != "") { ImportRunwaysFromCsv(_rwyCmb.Text); RegenerateRwyMemo(_rwyCmb.Text); RwyLoadGrid(_rwyCmb.Text); LogUserAction("Re-import RWYs CSV for " + _rwyCmb.Text); } };
			tabPage5.Controls.Add(bReimport);

			Button bAdd = new Button { Tag = "dispose", Top = 49, Left = 350, Size = new Size(90, 26), Text = "+ Add RWY" };
			bAdd.Click += (s, e) => { if (!EnsureWriterOrWarn()) return; if (_rwyDgv != null) { _rwyDgv.Rows.Add(); LogUserAction("Add RWY row for " + _rwyCmb.Text); } };
			...
			bSave.Click += (s, e) => { if (!EnsureWriterOrWarn()) return; if (_rwyCmb.Text != "") { RwySaveGrid(_rwyCmb.Text); RegenerateRwyMemo(_rwyCmb.Text); LogUserAction("Save RWYs for " + _rwyCmb.Text); } };
