...
- **Conflict definition**: a NOTAM at station X conflicts with a `FlightSchedule` flight when X is the flight's `Origin` and the NOTAM's `[startdate, enddate]` **overlaps** `[STD-2h, STD+2h]`, or X is the `Dest` and it overlaps `[STA-2h, STA+2h]` (`Overlaps` — simple interval-overlap test, not "NOTAM must span the whole window"). `LoadFsFlights()` reads the same `FlightSchedule` table the Flight Schedule tab already populates — the Conflict tab doesn't refresh it itself, so Flight Schedule/DB Update should be run first for current data.
...
- NOTAM `startdate`/`enddate` are parsed via the first 16 characters (`"yyyy-MM-dd HH:mm"`, same convention as `dateTransformation`); `FlightSchedule.STD`/`STA` are parsed as UTC (`DateTimeStyles.AdjustToUniversal|AssumeUniversal`) since they're stored `yyyy-MM-ddTHH:mm:ssZ`.
